...
public class EnzymeDigest {
    ...
    private static void create_crossLinkedPeptides(File output, String crossLinker, int minLen, int maxLen) throws IOException {
        DBLoader loader = DBLoaderLoader.loadDB(output),
                loader_next = null;
        Protein start_protein = null,
                next_protein = null;
        CrossLinkerName linker = CrossLinkerName.DSS;
        BufferedWriter bw = new BufferedWriter(new FileWriter(output.getName() + ".fastacp"));
